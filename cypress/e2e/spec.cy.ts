...
describe('can display the app', () => {
  beforeEach(() => {
    ...
    cy.intercept(
      "GET",
      "/api/stops/Ligne_D_Horizon_Vasacoss_R",
      { fixture: "stops.json" }
    ).as('stopsApi');

    cy.intercept(
      "GET",
      "/api/ways/Ligne_D_Horizon_Vasacoss_R",
      { fixture: "ways.json" }
    ).as('waysApi');
  ...
  it("Title of the page contains BetaX", () => {
    cy.get("title", { timeout: 1000 })
      .should("contain", "BetaX");
  });

  it("The bus line list is displayed", () => {
    cy.get("#root > div:nth-child(2) > div:nth-child(2) > ul", { timeout: 1000})
      .children(0)
      .should("contain", "Ligne_D_Horizon_Vasacoss_R");
  });

  it("The bus stop list is displayed", () => {
    cy.get('#root > div:nth-child(2) > div:nth-child(1) > ul')
      .children(0)
